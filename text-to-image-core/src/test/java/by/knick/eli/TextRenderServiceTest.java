...
public class TextRenderServiceTest {
	...
	private TextRenderServiceImpl testable = new TextRenderServiceImpl();
	...
	@Test
	public void renderDifferentLength() throws IOException {
		int charCount = 1;
		do {
			String text = textExample.substring(0, charCount);

			byte[] imgBytes = testable.renderText(text, "BMP");
	...
	@Test
	public void renderNonUtf() throws IOException {
		byte[] imgBytes = testable.renderText("β-Test", "BMP");
		Assert.assertNotNull(imgBytes);

		File imgBytesFile = File.createTempFile("text-render-unittest-renderNonUft-", ".bmp");
		FileUtils.writeByteArrayToFile(imgBytesFile, imgBytes);
	}
}
